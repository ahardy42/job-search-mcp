...
- `linkedin.ts` — Scrapes LinkedIn's public guest job search API via fetch + cheerio. Implements in-memory caching (1hr TTL), batch pagination (25 per page), exponential backoff on errors, and random user-agent rotation. Also exports `fetchJobDescription()` for scraping individual job pages (validates LinkedIn hostname + HTTPS).
